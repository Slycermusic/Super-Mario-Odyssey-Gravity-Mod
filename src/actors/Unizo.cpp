...
namespace {
    using namespace al;
    NERVE_IMPL(Unizo, Wait)
    NERVE_IMPL(Unizo, Wander)
    NERVE_IMPL(Unizo, Turn)
    NERVE_IMPL(Unizo, Find)
    NERVE_IMPL(Unizo, Chase)
    NERVE_IMPL(Unizo, Fall)
    NERVE_IMPL(Unizo, Land)
    NERVE_IMPL(Unizo, Attack)
    NERVE_IMPL(Unizo, CapHit)
    NERVE_IMPL(Unizo, BlowDown)

    struct {
        NERVE_MAKE(Unizo, Wait);
        NERVE_MAKE(Unizo, Wander);
        NERVE_MAKE(Unizo, Turn);
        NERVE_MAKE(Unizo, Find);
        NERVE_MAKE(Unizo, Chase);
        NERVE_MAKE(Unizo, Fall);
        NERVE_MAKE(Unizo, Land);
        NERVE_MAKE(Unizo, Attack);
        NERVE_MAKE(Unizo, CapHit);
        NERVE_MAKE(Unizo, BlowDown);

    } nrvUnizo;
}

Unizo::Unizo(const char *name) : al::LiveActor(name) { }

typedef void (Unizo::*functorType)();

void Unizo::init(al::ActorInitInfo const &info)
{
    al::initActorWithArchiveName(this, info, "Unizo", nullptr);
    al::initNerve(this, &nrvUnizo.Wander, 1);

    this->state = new EnemyStateWander(this, "Walk");
    al::initNerveState(this, this->state, &nrvUnizo.Wander, "徘徊");
...
void Unizo::listenAppear() {
    this->appear();
}

bool Unizo::receiveMsg(const al::SensorMsg* message, al::HitSensor* source,
                        al::HitSensor* target) {
    ...
    if(rs::isMsgCapReflect(message) && !al::isNerve(this, &nrvUnizo.BlowDown) && this->capHitCooldown <= 0) {
        rs::requestHitReactionToAttacker(message, target, source);
        al::setNerve(this, &nrvUnizo.CapHit);
        this->capPos = al::getSensorPos(source);
        this->capHitCooldown = 10;
        return true;
    }

    if((rs::isMsgBlowDown(message) || rs::isMsgDonsukeAttack(message)) && !al::isNerve(this, &nrvUnizo.BlowDown)) {
        ...
        rs::requestHitReactionToAttacker(message, target, source);
        al::setNerve(this, &nrvUnizo.BlowDown);
        return true;
    }
...
void Unizo::attackSensor(al::HitSensor* source, al::HitSensor* target) {

    if (!al::isNerve(this, &nrvUnizo.BlowDown)) {
        if (!al::sendMsgEnemyAttack(target, source)) {
            ...
            if (al::isNerve(this, &nrvUnizo.BlowDown) || al::isNerve(this, &nrvUnizo.Attack) ||
                 !al::sendMsgEnemyAttack(target, source)) {
                al::sendMsgPushAndKillVelocityToTarget(this, source, target);
                return;
            }
        }
        al::setNerve(this, &nrvUnizo.Attack);
    }
}
...
void Unizo::control() {
    ...
    if (al::isInDeathArea(this) || al::isCollidedFloorCode(this, "DamageFire") ||
        ...
        if(prevInt-- > 0) {
            ...
            if (prevInt == 0) {
                if(al::isNerve(this, &nrvUnizo.Wander)) {
...
void Unizo::exeWait(void)  // 0x0
{
    ...
    if (al::isValidStageSwitch(this, "SwitchStart") && al::isOnStageSwitch(this, "SwitchStart")) {
        al::setNerve(this, &nrvUnizo.Wander);
    }
    al::rotateQuatZDirDegree(this, 1.0f);
}

void Unizo::exeWander(void)  // 0x8
{
    if (al::isFirstStep(this)) {
        al::setVelocityZero(this);
    ...
    if (isGrounded && isNearPlayer) {
        al::setNerve(this, &nrvUnizo.Turn);
    } else if (isGrounded) {
        this->airTime = 0;
...
void Unizo::exeCapHit(void)  // 0x10
{
    ...
    if (al::isActionEnd(this)) {
        if (al::isNearPlayer(this, 1000.0f)) {
            al::setNerve(this, &nrvUnizo.Find);
        } else {
            al::setNerve(this, &nrvUnizo.Wander);
        }
...
void Unizo::exeBlowDown(void)  // 0x18
{
    if (al::isFirstStep(this)) {
...
void Unizo::exeAttack(void)  // 0x20
{
    if (al::isFirstStep(this)) {
...
void Unizo::exeFall(void)  // 0x30
{
    ...
    if (al::isOnGround(this, 0)) {
        this->airTime = 0;
        al::validateClipping(this);
        al::setNerve(this, &nrvUnizo.Land);
    }
}

void Unizo::exeFind(void)  // 0x38
{
    if (al::isFirstStep(this)) {
        al::setVelocityZero(this);
    ...
    if (!al::isOnGround(this, 0) && this->airTime++ >= 4) {
        ...
        if (!al::isActionEnd(this))
            return;
        al::setNerve(this, &nrvUnizo.Chase);
    }
}

void Unizo::exeChase(void)  // 0x40
{
    if (al::isFirstStep(this)) {
