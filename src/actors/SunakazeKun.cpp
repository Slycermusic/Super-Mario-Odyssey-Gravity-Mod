...
namespace {
    using namespace al;
    NERVE_IMPL(SunakazeKun, Wait)
    NERVE_IMPL(SunakazeKun, Wander)
    NERVE_IMPL(SunakazeKun, Turn)
    NERVE_IMPL(SunakazeKun, Find)
    NERVE_IMPL(SunakazeKun, Chase)
    NERVE_IMPL(SunakazeKun, Fall)
    NERVE_IMPL(SunakazeKun, Land)
    NERVE_IMPL(SunakazeKun, Attack)
    NERVE_IMPL(SunakazeKun, CapHit)
    NERVE_IMPL(SunakazeKun, BlowDown)

    struct {
        NERVE_MAKE(SunakazeKun, Wait);
        NERVE_MAKE(SunakazeKun, Wander);
        NERVE_MAKE(SunakazeKun, Turn);
        NERVE_MAKE(SunakazeKun, Find);
        NERVE_MAKE(SunakazeKun, Chase);
        NERVE_MAKE(SunakazeKun, Fall);
        NERVE_MAKE(SunakazeKun, Land);
        NERVE_MAKE(SunakazeKun, Attack);
        NERVE_MAKE(SunakazeKun, CapHit);
        NERVE_MAKE(SunakazeKun, BlowDown);

    } nrvSunakazeKun;
}

SunakazeKun::SunakazeKun(const char *name) : al::LiveActor(name) { }

typedef void (SunakazeKun::*functorType)();

void SunakazeKun::init(al::ActorInitInfo const &info)
{
    al::initActorWithArchiveName(this, info, "SunakazeKun", nullptr);
    al::initNerve(this, &nrvSunakazeKun.Wander, 1);

    this->state = new EnemyStateWander(this, "Walk");
    al::initNerveState(this, this->state, &nrvSunakazeKun.Wander, "徘徊");
...
void SunakazeKun::listenAppear() {
    this->appear();
}

bool SunakazeKun::receiveMsg(const al::SensorMsg* message, al::HitSensor* source,
                        al::HitSensor* target) {
    ...
    if(rs::isMsgCapReflect(message) && !al::isNerve(this, &nrvSunakazeKun.BlowDown) && this->capHitCooldown <= 0) {
        rs::requestHitReactionToAttacker(message, target, source);
        al::setNerve(this, &nrvSunakazeKun.CapHit);
        this->capPos = al::getSensorPos(source);
        this->capHitCooldown = 10;
        return true;
    }

    if((rs::isMsgBlowDown(message) || rs::isMsgDonsukeAttack(message)) && !al::isNerve(this, &nrvSunakazeKun.BlowDown)) {
        ...
        rs::requestHitReactionToAttacker(message, target, source);
        al::setNerve(this, &nrvSunakazeKun.BlowDown);
        return true;
    }
...
void SunakazeKun::attackSensor(al::HitSensor* source, al::HitSensor* target) {

    if (!al::isNerve(this, &nrvSunakazeKun.BlowDown)) {
        if (!al::sendMsgEnemyAttack(target, source)) {
            ...
            if (al::isNerve(this, &nrvSunakazeKun.BlowDown) || al::isNerve(this, &nrvSunakazeKun.Attack) ||
                 !al::sendMsgEnemyAttack(target, source)) {
                al::sendMsgPushAndKillVelocityToTarget(this, source, target);
                return;
            }
        }
        al::setNerve(this, &nrvSunakazeKun.Attack);
    }
}

void SunakazeKun::control() {
    ...
    if (al::isInDeathArea(this) || al::isCollidedFloorCode(this, "DamageFire") ||
        ...
        if(prevInt-- > 0) {
            ...
            if (prevInt == 0) {
                if(al::isNerve(this, &nrvSunakazeKun.Wander)) {
...
void SunakazeKun::updateCollider() {
...
void SunakazeKun::updateVelocity() {
...
void SunakazeKun::exeWait(void)  // 0x0
{
    ...
    if (al::isValidStageSwitch(this, "SwitchStart") && al::isOnStageSwitch(this, "SwitchStart")) {
        al::setNerve(this, &nrvSunakazeKun.Wander);
    }
    al::rotateQuatZDirDegree(this, 1.0f);
}

void SunakazeKun::exeWander(void)  // 0x8
{
    if (al::isFirstStep(this)) {
        al::setVelocityZero(this);
    ...
    if (isGrounded && isNearPlayer) {
        al::setNerve(this, &nrvSunakazeKun.Turn);
    } else if (isGrounded) {
        this->airTime = 0;
        ...
        if (this->airTime > 4) {
            al::setNerve(this, &nrvSunakazeKun.Fall);
        }
    }
}

// FIXME vector math is non-matching, but seems to be functionally identical.
void SunakazeKun::exeCapHit(void)  // 0x10
{
    sead::Quatf frontUp;
    ...
    if (al::isActionEnd(this)) {
        if (al::isNearPlayer(this, 1000.0f)) {
            al::setNerve(this, &nrvSunakazeKun.Find);
        } else {
            al::setNerve(this, &nrvSunakazeKun.Wander);
        }
...
void SunakazeKun::exeBlowDown(void)  // 0x18
{
    if (al::isFirstStep(this)) {
...
void SunakazeKun::exeAttack(void)  // 0x20
{
    if (al::isFirstStep(this)) {
...
void SunakazeKun::exeTurn(void)  // 0x28
{
    if(al::isFirstStep(this)) {
        al::setVelocityZero(this);
...
void SunakazeKun::exeFall(void)  // 0x30
{
    if (al::isFirstStep(this)) {
        al::invalidateClipping(this);
    ...
    if (al::isOnGround(this, 0)) {
        this->airTime = 0;
        al::validateClipping(this);
        al::setNerve(this, &nrvSunakazeKun.Land);
    }
}

void SunakazeKun::exeFind(void)  // 0x38
{
    if (al::isFirstStep(this)) {
        al::setVelocityZero(this);
    ...
    if (!al::isOnGround(this, 0) && this->airTime++ >= 4) {
        ...
        if (!al::isActionEnd(this))
            return;
        al::setNerve(this, &nrvSunakazeKun.Chase);
    }
}

void SunakazeKun::exeChase(void)  // 0x40
{
    if (al::isFirstStep(this)) {
...
void SunakazeKun::exeLand(void)  // 0x48
{
    int* airTimePtr;
