...
static PlanetGravityManager* pManager = new PlanetGravityManager("PlanetGravityManager");
...
namespace {
	...
	bool calcGravityVectorOrZero(const LiveActor *pActor, const TVec3f &rPosition, u32 gravityType, TVec3f * pDest, GravityInfo *pInfo, u32 host) {
		if (!host) {
			host = *reinterpret_cast<u32*>(&pActor);
		}
...
namespace MR {
	...
	void resetGravityManager() {
		delete pManager;
		pManager = new PlanetGravityManager("PlanetGravityManager");
	}
}
